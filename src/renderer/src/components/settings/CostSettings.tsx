...
export function CostSettings() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setSummary(s); setBudget(b); setRecords(r); setPricing(p); setForm(b ?? {})
    ...
    <div className="max-w-2xl mx-auto p-6 space-y-6">
      ...
      {summary && (
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-surface border border-border rounded-xl p-4">
            <div className="text-xs text-text-muted">{t('ap.cost.totalTokens')}</div>
            <div className="text-2xl font-bold text-text-primary mt-1">{summary.totalTokens.toLocaleString()}</div>
          </div>
          <div className="bg-surface border border-border rounded-xl p-4">
